...
- [ ] `UIButtonDetector` : screenshot → `{name: (x,y,conf)}`. Entraînement = script **générique** `tools/train/kaggle_train_yolo_troops.py` (lit n'importe quel dataset Roboflow). **Data + label = côté utilisateur** (captures des écrans à boutons, Roboflow) — pas d'outil de collecte à faire. **La bascule = un seul `set_detector()` au démarrage**, zéro appelant à modifier.
- [ ] **Migrer les appelants restants vers `find_button()`** (17 appels directs à `get_position` + 18 taps en dur hors JSON) : retraite → dialogues confirmer/annuler → demander château → menus bâtiment. Sans urgence : le seam existe, ces appelants marchent déjà via la calibration.
