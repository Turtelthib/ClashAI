...
- [x] **Split BONUS (hors plan initial)** : `combat/spell_caster.py` → `combat/spell_caster/` (same-name : constants, health_bars, clustering, caster, __main__). API ré-exportée : `SpellCaster, detect_health_bars, cluster_positions`. Note : ce module garde son propre `detect_health_bars`/`cluster_positions` (variante avec `points` + wrap rouge 170-180), distincts de ceux de `combat_observer` — gardés séparés pour ne pas changer le comportement.

> ✅ **REFACTO PHASE 3 TERMINÉE** : 13 fichiers splittés (12 du plan + spell_caster bonus) + migration CNN héros + 3 bugs préexistants corrigés (weights_dir GdC, entry point brain cassé, NO_TROOPS dupliqué). **Critère de réussite atteint : 0 fichier >500L hors `legacy/`.** Prochaine étape : diagnostiquer les capacités héros jamais déclenchées (🚩 plus haut).
