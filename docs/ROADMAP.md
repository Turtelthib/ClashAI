...
- [ ] **Anti-gemmes — voir `declined` en jeu** (correctif du 12 sept. 2026) : l'annulation sûre est vue en réel (canon verrouillé HDV 15 → `need_decision`, aucune dépense). Reste à voir `declined` sur un achat **payable** : démo upgrade sans `--confirm` sur un bâtiment non maxé.
...
- [ ] 🔴 **Ouvriers mal lus sur un 2ᵉ compte (12 sept. 2026) — touche l'anti-gemmes.** Écran « 5/5 », lu **4/5** (conf 0.87). La segmentation trouve **5 glyphes** au lieu de 2 (`['4','4','5','7','5']`) : le crop contient aussi la pastille bleue « i » et le sourire blanc de l'ouvrier, et `read_widget_ratio` prend le PREMIER glyphe. Le garde-fou d'homogénéité n'a rien refusé. **Risque** : compter des ouvriers libres qui n'existent pas contourne `no_builder` — et sans ouvrier, le jeu propose de finir un chantier en gemmes. Piste : lire le ratio depuis la DROITE (M = dernier glyphe, N = celui qui précède le « / »). Crop : `debug_upgrade/upgrade_1_village_ouvriers_crop.png`.
- [ ] **Amélioration verrouillée par l'HDV non reconnue** : « Vous devez améliorer votre hôtel de ville au niveau 15 » → le jeu grise `confirmer_upgrade` et la pièce du prix (saturation mesurée : 0). Aujourd'hui → `need_decision` : sûr, mais **par chance** (la pièce grise empêche d'identifier la ressource). Détecter le bouton grisé par saturation, comme les boutons `donner` → statut explicite `locked`.
- [ ] **CNN bâtiments : labo niveau 12 non reconnu** (compte niveau 148) → `lab_not_found`. Classé `defense_antiaerienne` 94 %, `laboratoire` **0,0 %** ; retesté menu fermé, même résultat → ce n'est pas la sélection, c'est un skin absent du dataset.
