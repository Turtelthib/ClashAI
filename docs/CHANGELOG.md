...
- ✅ **Agent village — incrément 2 : upgrades (mains + yeux pour le LLM)** — la **décision** (quoi améliorer) reviendra au LLM (V5.3) ; on livre le **geste** + les **capteurs proactifs**, conçus comme des outils appelables.
  ...
  - **14 tests** (`test_widget_reader.py` + `test_village_upgrader.py`, digit CNN + I/O mockés) : lecture ressources/ouvriers, scaling ADB→image, chaque statut, et l'invariant **anti-gemmes** (`confirmer` jamais tapé sans décision). **176 tests.**
