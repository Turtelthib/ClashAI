...
- ✅ **Digit CNN — intégration reset-seeding (Phase 4)** : `core._seed_counts_from_digits()` lit la barre de combat au début de l'attaque (compteurs pleins, position combat = matché à l'entraînement) et seede `_remaining_troops` avec les **vrais compteurs** → l'agent connaît son armée (deploy précis, fini le `default_max` approximatif). Fallback par troupe si conf basse ; sorts exclus (cast-until-grayed). `digit_reader.crop_count_badge`/`read_bar_counts` = SSOT partagé avec collect. Modèle retrainé sur data 0/7 enrichie : **100% val acc par classe**. À valider émulateur.
