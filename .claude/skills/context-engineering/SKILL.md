...
Le skill amont référençait 15 sous-skills (`skills/context-fundamentals/SKILL.md`, `multi-agent-patterns`, `memory-systems`, `tool-design`, `filesystem-context`, `hosted-agents`, `context-optimization`, `latent-briefing`, `evaluation`, `advanced-evaluation`, `harness-engineering`, `project-development`, `bdi-mental-states`, `context-degradation`, `context-compression`). Aucun n'a été fourni avec la copie installée ici — les renvois ont été retirés. Le contenu ci-dessus (la carte des concepts) reste utilisable seul ; pour le détail, reprendre la collection complète chez l'auteur.
